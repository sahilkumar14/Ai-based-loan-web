...
export default function Login({ setRole, setUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (matched) {
      ...
      console.debug("[Login] matched dummy user role:", matched.role);
      // Optional: attach any user data to localStorage for other pages
      try { 
        localStorage.setItem('devUser', JSON.stringify(matched)); 
        localStorage.setItem('user', matched.name);
  ...
  console.debug("[Login] server login response:", data);
      if (data.success) {
        // Prefer the role returned by server if present; otherwise fall back to the selected role
        const roleToUse = data.role || form.role;
        if (setRole) setRole(roleToUse);
        // Ensure a visible user string (fallback to email)
        const userName = data.name || form.email || roleToUse;
        if (setUser) setUser(userName);
        try {
          localStorage.setItem('user', userName);
          localStorage.setItem('role', roleToUse);
        } catch {}
        // notify other listeners in same tab that auth changed (storage events don't fire in same tab)
        try {
          window.dispatchEvent(new CustomEvent('authChanged', { detail: { user: userName, role: roleToUse } }));
        } catch {}
        console.debug("[Login] using role after login:", roleToUse, "user:", userName);
        navigate(roleToUse === "student" ? "/student" : "/distributor");
      } else {
